...
using ClimateCamp.EntityFrameworkCore.Seed.Host;
using ClimateCamp.EntityFrameworkCore.Seed.Tenants;
...
{
    public abstract class ClimateCampTestBase : AbpIntegratedTestBase<ClimateCampTestModule>
    {
        protected ClimateCampTestBase()
        {
            void NormalizeDbContext(CommonDbContext context)
            {
        ...
        protected void UsingDbContext(Action<CommonDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<CommonDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<CommonDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<CommonDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<CommonDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<CommonDbContext>())
                {
                    action(context);
                    context.SaveChanges();
                }
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<CommonDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<CommonDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<CommonDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<CommonDbContext, Task<T>> func)
        {
            T result;
        ...
        {
            LoginAsHost(ClimateCampConsts.AdminUserName);
        }

        protected void LoginAsDefaultTenantAdmin()
        {
            LoginAsTenant(AbpTenantBase.DefaultTenantName, ClimateCampConsts.UserAdminUserName);
        }
